	...
	private void initialize () {
		addListener(inputListener = new ClickListener() {
			...
			public boolean touchDown (InputEvent event, float x, float y, int pointer, int button) {
				...
				if (stage != null) stage.setKeyboardFocus(ChatTextField.this);
				keyboard.show(true);
				return true;
			}
			...
			public boolean keyDown (InputEvent event, int keycode) {
				...
				Stage stage = getStage();
				if (stage != null && stage.getKeyboardFocus() == ChatTextField.this) {
...
//						next(Gdx.input.isKeyPressed(Keys.SHIFT_LEFT) || Gdx.input.isKeyPressed(Keys.SHIFT_RIGHT));
					...
					if (listener != null) {
						// CODE I ADDED TO MAKE ENTER WORK - Leejjon
						if ((int)character == 0 && event.getKeyCode() == 66) {
							character = '\r';
						}
	...
	private TextField findNextTextField (Array<Actor> actors, TextField best, Vector2 bestCoords, Vector2 currentCoords, boolean up) {
		for (int i = 0, n = actors.size; i < n; i++) {
			...
			if (actor instanceof TextField) {
				ChatTextField textField = (ChatTextField)actor;
	...
	static public interface TextFieldFilter {
		...
		public boolean acceptChar (ChatTextField textField, char key);
